...
list_of_packages <- c("R6", "data.table", "stringr", "dplyr", "tibble", "docstring", "lubridate", "readr")
...
PostcodeGeocoder <- R6Class("PostcodeGeocoder",
                            public = list(
                              ...
                              extract_pcds = function(column, pcd_data = NULL, ...){
                                ...
                                stringr::str_c("Extracted ", sum(!is.na(tb$ext_pcds)), " out of ", length(tb$ext_pcds), " postcodes") %>% message()
                                return(tb)
                              },
                              geocode_pcds = function(column, pcd_data = NULL, ...){
                                ...
                                stringr::str_c("Geocoded  ", sum(!is.na(tb_geo$lat)), " out of ", sum(!is.na(tb$ext_pcds)), " postcodes") %>% message()
                            ...
                            private = list(
                              get_data = function(pcd_data = NULL, ...){
                                ...
                                if (is.null(pcd_data)){
                                  ...
                                  tb <- data.table::fread(file = file, ...)
                                } else if (! is.data.frame(pcd_data)){
                                  tb <- data.table::fread(file = pcd_data, ...) 
                                } else{
                                  tb <- pcd_data
                                }
                              ...
                              read_nspl = function(nspl_csv = NULL){
                                ...
                                import_nspl <- function(nspl_path){
                                  self$postcodes <- data.table::fread(file = nspl_path, select = self$nspl_cols) %>% 
                                    tibble::as_tibble()
                                }
